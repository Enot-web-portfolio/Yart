...
export namespace WorksApi {
  ...
  export async function getWorks(page: number, count: number, onlySubscriptions: boolean, userId?: number | string, mainSkills?: number[] | string[]): Promise<Work[]> {
    const url = `${CONFIG.apiUrl}/works?page=${page}&count=${count}&onlySubscriptions=${onlySubscriptions}${userId !== undefined ? `&userOuterId=${userId}` : ''}${mainSkills !== undefined && mainSkills.length > 0 ? `&skillIds=${mainSkills.join(', ')}` : ''}`;
